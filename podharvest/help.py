...
PURPOSES: dict[str, str] = {
    ...
    "Import a list of podcasts": (
        ...
        "web address or choose a file, press Read the list, then work down "
        "it with the arrow keys and Space to check the shows you want. "
        "Space only ever checks: it never leaves this window. Enter adds "
        "everything you have checked, and the Add button says how many that "
        "is. Check the new ones skips anything already in your favourites, "
        "which is what you usually want when re-reading a list. Importing "
        "adds bookmarks and nothing else: no subscription, no checking for "
        "new episodes, no downloading. Use this one now takes the "
        "highlighted show straight to the main window instead."
    ),
    "Chapters - ": (
    ...
    "Favourite podcasts": (
        ...
        "highlighted show's feed back to the main window, and Delete or "
        "Remove takes it off this list without touching anything you have "
        "already harvested from it. The Filter box at the top narrows the "
        "list to names containing what you type, which is the quick way "
        "into a long one, and the box below the list gives the highlighted "
        "show's feed address. These are bookmarks rather than "
        "subscriptions: podHarvest never checks them for new episodes or "
        "downloads anything on its own."
    ),
    "podHarvest": (
        "The main window. Start at the Source group: Find a podcast searches "
        "Apple's directory by name, Podcast feed takes an address you already "
        "have, Podcast list reads an OPML list of shows, and Local files "
        "works on audio already on this machine. Each has its own box below "
        "it, and the Chosen podcast line underneath says what the rest of the "
        "window is about -- everything that acts on a podcast stays greyed "
        "until one is chosen. Then press Start; everything else has a "
        ...
        "Control+T opens the selected episode's tags and chapters, Control+K "
        "goes to the search box, and Control+Shift+E lists what a feed holds "
