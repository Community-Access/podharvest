...
| Timings are read from transcripts you already have | Every timing feature works on a library harvested before this version existed, because podHarvest has always written `[HH:MM:SS.mmm]` markers into transcripts and now reads them back. Nothing has to be transcribed again to gain search-to-the-second, clips or phrase-placed chapters. |
| The big files can live elsewhere, the small ones cannot | Models and downloads can be moved to another drive from Settings, which shows what each folder costs and how much room the destination has. Settings and logs deliberately stay put: a settings file that moves is one you can lose, and the log has to be readable when the thing you are reporting is that the other folder broke. |
