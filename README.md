...
[![Accessibility statement](https://img.shields.io/badge/accessibility-statement-blueviolet)](docs/ACCESSIBILITY.md)
...
- **Keyboard-first and screen-reader-aware** — with an [accessibility statement](docs/ACCESSIBILITY.md) that states plainly what has been verified and what has not.
...
**New to podharvest?** [`docs/GETTING_STARTED.md`](docs/GETTING_STARTED.md) walks through installing, your first fetch, and your first transcription step by step.
...
podharvest never calls a cloud transcription API. Everything below runs locally and is downloaded on first use straight into the portable app space (see [`docs/MODELS.md`](docs/MODELS.md) for the full catalogue, licenses, and technical notes).
...
Real example from this machine (CPU-only, 3 faster-whisper sizes against the same 5-minute clip, scored against a distil-large-v3-generated reference):

| Model | Speed | WER | Accuracy |
|---|---|---|---|
| tiny.en | 26.9x real-time | 4.2% | 95.8% |
| base.en | 16.6x real-time | 4.2% | 95.8% |
| small.en | 6.3x real-time | 3.4% | 96.6% |
...
Accessibility is a functional requirement here, and this section is deliberately specific about what has been verified versus what has not. **No manual screen reader pass has been performed yet** — [`docs/ACCESSIBILITY.md`](docs/ACCESSIBILITY.md) says so plainly and lists every known gap. Running one and reporting what you find is the most useful contribution anyone could make to this project right now.
...
podharvest is [MIT licensed](LICENSE). Models it can download each carry their own license — see [`docs/MODELS.md`](docs/MODELS.md) before using any of them commercially.
